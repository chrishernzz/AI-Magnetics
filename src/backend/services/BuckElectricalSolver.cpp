...
//precondition: input.topology == Topology::Buck; vinMinV, vinMaxV, ioutA,
//switchingFreqKHz, rippleCurrentPercent are all positive; vinMinV <= vinMaxV;
//vinMaxV > voutV > 0 (a buck converter cannot regulate Vout >= Vin)
//postcondition: returns an InductorDesignRequest with inductanceUH,
//peakCurrentA, switchingFreqKHz, averageCurrentA, and
//rippleCurrentPeakToPeakA populated - rmsCurrentA is left unset so
//RequirementDerivationService derives it downstream using the exact same
//triangular-ripple formula Mode 2 already relies on
InductorDesignRequest BuckElectricalSolver::solve(const TopologyInput& input) {
    ...
    if (input.vinMinV <= 0.0 || input.vinMaxV <= 0.0 || input.ioutA <= 0.0 ||
        input.switchingFreqKHz <= 0.0 || input.rippleCurrentPercent <= 0.0) {
    ...
    //ideal duty cycle, no diode/switch drop modeling in V1
    ...
    out.peakCurrentA = input.ioutA + rippleTargetA / 2.0;
    ...
    //rmsCurrentA intentionally left unset - RequirementDerivationService
    //derives it from these two using the same triangular-ripple formula
    //Mode 2 already uses, so there is exactly one RMS derivation in the
    //whole codebase regardless of which mode produced the request.
