...
//One operating point's derived electrical quantities, evaluated at a
//specific Vin using the inductance value that was actually sized (at
//Vin_max) - these are NOT independently re-sized per point, they show how
//the one physical inductor behaves across the input range.
struct BuckOperatingPointResult {
    double vinV = 0.0;
    double dutyCycle = 0.0;
    double rippleCurrentPeakToPeakA = 0.0;
    double peakCurrentA = 0.0;
    //Iout - ripple/2 at this Vin, using the fixed sized inductance - the
    //quantity CCM/DCM classification is based on (see conductionMode).
    double minInductorCurrentA = 0.0;
};

struct BuckSolveResult {
    //sized at the worst-case point for ripple current (Vin_max) - the exact same
    //InductorDesignRequest Mode 2 accepts directly, so nothing downstream changes.
    InductorDesignRequest request;

    BuckOperatingPointResult atVinMin;
    BuckOperatingPointResult atVinMax;

    //"VinMin" | "VinMax" - which operating point is worst for each quantity,
    //since it need not be the same point for all three.
    ...
    //Stated Phase 1 assumptions (ideal converter, CCM, triangular ripple,
    //no PCB/connector drop) - never silently assumed without being surfaced.
    std::vector<std::string> assumptions;

    //Non-fatal notices (e.g. CCM-boundary) - conditions worth flagging that
    //do not by themselves make the request invalid.
