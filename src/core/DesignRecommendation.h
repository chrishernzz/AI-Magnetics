...
struct DesignRecommendation {
    std::string status;  // "ok" | "no_feasible_design"
    std::string message;
    std::vector<InductorCandidate> candidates;         // passed, ranked best first
    std::vector<InductorCandidate> rejectedCandidates;  // evaluated but did not pass every check
    DesignRules activeRules;

    // Populated only when status == "no_feasible_design" and the failure
    // is an area-product shortfall (spec section 8's structured result).
