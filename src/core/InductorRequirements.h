...
//
// This is the C++-side counterpart of the API's InductorDesignRequest,
// after RequirementDerivationService has converted units (uH -> H, kHz ->
// Hz) and derived any values that can be derived (e.g. rmsCurrentA from
// average current + ripple, for a triangular ripple waveform). Every stage
// downstream of RequirementDerivationService reads from this struct, never
// from the raw request, so there is exactly one place unit conversion and
// current-field derivation happens.
...
struct InductorRequirements {
    ...
    // Optional inputs (spec section 5) - present only if the request
    // supplied them. Nothing downstream may invent a value for these.
