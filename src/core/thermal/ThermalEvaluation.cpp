...
ThermalEvaluationResult evaluateThermal(const ThermalIterationInputs& inputs, const DesignRules& rules) {
    ...
    if (!converged) {
        ...
        // is not a meaningful "preliminary estimate" in that case, so it is not reported as one.
        result.status = ThermalStatus::NotEvaluated;
        result.missingDataExplanation =
            ...
            "operating point rather than a benign numerical issue; no preliminary estimate is reported";
        return result;
    }
