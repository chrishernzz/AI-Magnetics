...
//function for core area product to tell us whether a magnetic core has enough area to carry the magnetic flux and window area to fit
static double coreAreaProductCm4(double aeMm2, double waMm2) {
    //Ap = Ae x Aw
    return (aeMm2 * waMm2) * 1e-4;
}

//function to estimate copper loss (simplifeid: based on core size proxy)
static double estimateCopperLossW(const CoreData& core, double peakCurrentA) {
    //Rough heuristic: larger core = less loss
    //Loss ~ I^2 x (core_size)^2(-0.5)
...
CoreSelectionResult selectCore(const CoreSelectionInput& input) {
    ...
    //using material-matched cores if available, otherwise use all candidates
    ...
    for(const auto* core : finalCandidates) {
        ...
        std::cout << "  " << core->partNumber << ": loss=" << loss;
        if(loss < bestLoss) {
            std::cout << " ← NEW BEST";
            bestLoss = loss;
            bestCore = core;
    ...
    std::cout << "\nSelected: " << bestCore->partNumber << " (material=" << bestCore->material << ", loss=" << bestLoss << ")" << std::endl;
    std::cout << "========================\n" << std::endl;
    
    return {bestCore->partNumber, bestCore->material, bestCore->mu, bestCore->al, 
            bestCore->ae, bestCore->wa, bestCore->le};
}