...
| RecommendationStatus | `src/validation/RecommendationStatus.cpp` | ✅ Implemented — 3-tier `Phase1Recommended`/`PreliminaryCandidate`/`Rejected` classification replacing the old frontend-only "Recommended" sugar; `Phase1Recommended` currently unreachable in practice (ThermalValidation always flags isPreliminaryEstimate) |
