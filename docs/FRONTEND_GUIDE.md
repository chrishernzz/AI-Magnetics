...
**In Buck mode**, the Diagnostics card also draws a small real waveform - the inductor current over one switching period (a triangle wave: rises for the duty-cycle fraction of the period, falls for the rest) - directly from the same live `dutyCycle`/`peakCurrentA`/`rippleCurrentPeakToPeakA` numbers shown as text above it. It's not a static illustration: change any Buck input and it redraws from the current derived values, same as the numbers next to it. The chart updates the same SVG elements in place rather than rebuilding them from scratch, so the trace, its filled area, and the peak marker morph smoothly to the new shape instead of snapping between two static pictures - a small pulsing dot on the most recent peak reads as a live trace rather than a drawing. The vertical scale always fills the chart height regardless of the actual current magnitude (an autoscale, the way a real oscilloscope would) - the peak/min labels are what actually carry the magnitude; the trace's horizontal shape moves when duty cycle changes.
