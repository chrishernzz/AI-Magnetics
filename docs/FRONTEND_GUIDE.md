...
**Validation checks display**: the summary line shows the split before you even open it (e.g. `Validation checks (5 pass, 1 not evaluated)`). Once open, checks lay out two per row as a compact `status · name · value / limit` line — a passing check's number next to its limit already says everything a pass needs to say. Only checks that need attention (fail or not evaluated) get their full explanation sentence and span the full row width, so a mostly-passing candidate's detail view stays short instead of repeating six full sentences every time.
