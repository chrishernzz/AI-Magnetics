...
Core loss is `not_evaluated` for every candidate today — real Steinmetz coefficients exist in `data/real_core_loss_coefficients.csv` for 17 of 32 materials, but `CoreLoss.cpp` isn't wired to use them yet. DC copper loss and DCR are now real for cores with a usable mean-length-per-turn estimate (`data/real_cores.csv`'s `Mlt` column) — `not_evaluated` only for the subset of cores whose upstream geometry doesn't support that estimate. See [DATA_FILES.md](DATA_FILES.md) — these are data/wiring gaps, not silent bugs, and the tool says so explicitly via `missingData`/`missingDataWarnings`, surfaced in the UI as amber "not evaluated" chips rather than blank cells or fake zeros.
