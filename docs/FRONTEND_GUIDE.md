...
### Candidates table
One row per evaluated candidate — passing and rejected together, filterable to All / Passing / Rejected, and (added after a real user report that the tool had no way to identify or search for a toroidal core) a second shape filter dropdown next to it — All shapes / Toroid / Two-Piece Set — only showing the shapes actually present in the current result. Each core's real shape gets its own **Shape** column (not a badge crowded next to the part number - a second real user report about the table feeling cluttered led to splitting it out, the same way Material already has its own column). Hover the shape chip to see the geometry family, e.g. "T", "ETD", "PQ". Rows are in a fixed order: passing candidates first (in the backend's own rank order, lowest known-evaluated-loss first), then rejected. There's no per-column sort control — the tool always names one specific recommended candidate, and re-sorting by another column can't change which one that is, so a sortable header would just be a different view of the same fixed recommendation rather than a real choice. Any field the engine reports `not_evaluated` for (DC copper loss, core loss) shows as an amber "not evaluated" chip in the table rather than a blank or a fabricated 0. The **Status** column shows a PASS/REJECT chip; a **Recommended** tier chip appears next to PASS only when a candidate actually reaches that tier — "Preliminary" (currently true of every passing candidate, since `Phase1Recommended` is structurally unreachable until real per-core thermal data exists - see FORMULAS.md section 12) is deliberately not shown here, since tagging every single passing row with the same word is noise, not a signal; its real meaning is still shown in the detail panel.

Clicking a row opens its full detail in a **slide-in side panel** from the right edge of the screen (added after a real user report that the old in-table accordion crammed too much into one long stacked block). The table stays visible and scrollable behind the panel. Close it with the × button, by clicking the dimmed overlay, or with Escape. Clicking a different row while the panel is open just re-renders its contents rather than stacking a second panel.

**Candidate detail panel layout**: the panel header repeats the core part number, its real shape, material, and pass/reject state. The body leads with a 5-box KPI strip (Known Evaluated Loss, Core Loss, Fill Factor, Current Density, Turns/Gap) — the numbers a candidate is actually judged by, before any narrative text. Below that is a one-line status (`Rejected — 3 of 6 checks failed`, or the real recommendation-tier chip plus `— N of M applicable checks passed`) — just the outcome, not a restatement of any individual check; this is where the "Preliminary" tier and its meaning actually show up, since it's genuinely useful once you're looking at one candidate's detail. Every check is then listed once, always visible (not collapsed): a colored row (red/green/amber) with a status chip, the check name, `actual <value> · limit <value> <unit>` labeled explicitly rather than a bare ratio, and its one real explanation sentence directly under its own row — a pass gets the same sentence a fail would (e.g. `peak flux density 0.310 T vs limit 0.501 T`), not just the numbers, since that sentence is what actually shows the check held, not only that it failed. Nothing about any check is ever stated in two different places — there's exactly one row per check, and that row is the only place its numbers and its reason live.

There is no separate summary card repeating the top candidate's material, core, turns/gap, and loss above the table — that was a second copy of exactly what the **Recommended** row (and its side panel) already shows, so it was removed rather than kept as a duplicate.
