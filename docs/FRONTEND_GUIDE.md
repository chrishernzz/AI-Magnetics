...
**Validation checks display**: collapsed by default, with the summary line showing the split before you even open it (e.g. `Validation checks (5 pass, 1 not evaluated)`). Checks lay out two per row: status, name, then `actual <value> · limit <value> <unit>` (explicitly labeled, not a bare `X / Y` pair). A failed check that's already a rejection reason shows neither its numbers nor its explanation here — both are the exact same fact already stated once, in full, in the verdict banner above — the line reads `see rejection reasons above` instead. The checks grid only adds detail the banner didn't already give: the full explanation for a `not evaluated` check, and the actual/limit numbers for every check not already covered above. Nothing is stated twice.
