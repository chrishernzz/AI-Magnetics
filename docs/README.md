...
- ✅ 3-tier recommendation (`src/validation/RecommendationStatus.cpp`) — `Phase1Recommended`/`PreliminaryCandidate`/`Rejected` replaces the old frontend-only "Recommended" sugar. Candidates are ranked by tier first, then real known evaluated loss (copper + core, whichever are `Evaluated`), predicted temperature rise, manufacturability margin, saturation margin, current-density margin, area product, and part number as a deterministic tiebreak — a missing number always ranks as the worst case for that tiebreaker, never the best. `Phase1Recommended` is currently unreachable in practice (ThermalValidation always flags its result as preliminary) — see [FORMULAS.md](FORMULAS.md) section 12
...
- ✅ Thermal evaluation (`src/core/thermal/ThermalEvaluation.cpp`) — real iterative convergence loop (temp → hot DCR → copper loss → temp rise → repeat); caps at `PreliminaryThermalEstimate` since `defaultThermalResistanceCPerW` is always a Phase 1 default, never per-core data. `not_evaluated` when DCR geometry is unknown or the loop's real positive-feedback iteration diverges
