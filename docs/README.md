...
| **Ask a local LLM questions (software docs or magnetics engineering knowledge)** | [RAG_ASSISTANT.md](RAG_ASSISTANT.md) |
...
- ✅ Natural-language input (`POST /parse-requirements`, `python/services/requirement_parser.py`) — describe the inductor in plain English and the local LLM (schema-constrained JSON extraction, LM Studio) fills the form; deterministic Python validates (Irms ≤ Ipk, plausibility ranges) and asks clarifying questions for missing fields instead of guessing. Local-machine only — requires LM Studio; the hosted site reports it unavailable (see [FRONTEND_GUIDE.md](FRONTEND_GUIDE.md))
